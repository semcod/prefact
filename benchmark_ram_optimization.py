...
def main() -> None:
    ...
    for num_files, file_size_kb in test_cases:
        ...
        try:
            ...
            all_results.append(
                {"num_files": num_files, "file_size_kb": file_size_kb, **results}
            )
    ...
    print("=" * 60)
    print(
        f"{'Files':<6} {'Size':<6} {'Without':<10} {'With':<10} {'Improvement':<12} {'Speedup':<8}"
    )
    print("-" * 60)
    ...
    # Average improvement
    if all_results:
        avg_improvement = sum(r["improvement_percent"] for r in all_results) / len(
            all_results
        )
