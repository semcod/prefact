...
class UnusedImports(BaseRule):
    ...
    def _remove_unused_from_line(self, line: str, used_names: list[str]) -> str:
        ...
        if match:
            ...
            filtered = [
                imp for imp in imports if any(name in imp for name in used_names)
            ]
            if filtered:
    ...
    def _remove_unused_from_import_line(
        self, line: str, used_aliases: list[str]
    ) -> str:
        ...
        if match:
            ...
            imports = [imp.strip() for imp in imports_part.split(",")]
            filtered = [
                imp for imp in imports if any(alias in imp for alias in used_aliases)
            ]
            if filtered:
                return prefix + ", ".join(filtered) + "\n"
            else:
