...
### Zmierzone wyniki przed / po optymalizacjach

| Próba | Przed (baseline) | Po Fix 1+2 | Po Fix 1+2+3 | Łączne przyśp. |
|---|---|---|---|---|
| `scan 50×1KB` | 16.4 s ❌ | 4.5 s ✅ | 4.5 s ✅ | **3.6×** |
| `scan 100×1KB` | 35.1 s ❌ | 6.6 s ✅ | 9.1 s ✅ | **3.9×** |
| `scan 200×5KB` | 72.1 s ❌ | 13.5 s ✅ | 17.2 s ✅ | **4.2×** |
| throughput 20×10 in-process | 70.5 s ❌ | 13.7 s ✅ | 18.1 s ✅ | **3.9×** |

> Fix 3 (AST cache) eliminuje wielokrotne `ast.parse()` per plik. Jego wpływ na benchmark jest niewielki w porównaniu z Fix 1+2, bo koszt samego parsowania AST jest mały względem subprocess overhead. Korzyść Fix 3 rośnie wraz z liczbą aktywnych reguł AST-based na plik.
