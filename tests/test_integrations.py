...
""",
                expected_issues={
                    "unused-imports": [{"line": PORT_3, "import": "json"}],
                    "duplicate-imports": [{"line": PORT_6, "import": "Dict"}],
                    "string-concat": [{"line": CONSTANT_9}],
                    "print-statements": [{"line": CONSTANT_9}],
                    "unused-variables": [{"line": CONSTANT_11, "variable": "x"}],
                    "missing-return-type": [{"line": CONSTANT_8}],
                },
            ),
...
from typing import List
""",
                expected_issues={"sorted-imports": [{"line": 1}]},
            ),
...
from os.path import *
""",
                expected_issues={"wildcard-imports": [{"line": 1}, {"line": 2}]},
            ),
...
""",
                expected_issues={
                    "missing-return-type": [{"line": 1}, {"line": CONSTANT_5}]
                },
            ),
        ]
    ...
    def run_all_tests(self, config: Config) -> Dict[str, Dict]:
        ...
        for test_case in self.test_cases:
            ...
            comparison = self._compare_results(test_case.expected_issues, actual_issues)
            ...
            results[test_case.name] = {
                ...
                "passed": all(
                    all(
                        issue.get("passed", False) for issue in rule_comparison.values()
                    )
    ...
    def _compare_results(
        self, expected: Dict[str, List[Dict]], actual: Dict[str, List[Issue]]
    ) -> Dict[str, Dict]:
        ...
        for rule_id in expected:
            ...
            for act_issue in actual_issues:
                if not any(
                    self._issues_match(exp, act_issue) for exp in expected_issues
                ):
